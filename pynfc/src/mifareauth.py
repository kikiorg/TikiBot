...
#    card_timeout = 10
    ...
    def __init__(self, logger):
        self.__context = None # Contains various settings for the device.
        self.__device = None # The device being polled
        self.log = logger # Log errors and messages -- set to log level 3 for verbose messages

        self._card_present = False
        self._card_last_seen = time.mktime(time.gmtime()) # A timestamp when the card was last seen
        self._card_uid = None
        self._clean_card() # Sets UID to None
        self.time_polling = time.mktime(time.gmtime())

        mods = [(nfc.NMT_ISO14443A, nfc.NBR_106)] # Poll for a Mifare standard tag
...
#                uid = "".join([chr(nt.nti.nai.abtUid[i]) for i in range(4)])
    ...
    def read_card(self, uid):
        ...
        self.log("Kiki: Reading card", uid.encode("hex"))
        self._card_uid = self.select_card()
        self.log("Kiki: type(UID) after self.select_card: ", type(self._card_uid))
