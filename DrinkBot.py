...
for each_ingr in recipe_book.fieldnames:
	ingr_list.append(each_ingr)
# This is the upper left entry.  Meaning, this is the title of the first column -- which is the drink names
# If all goes well, this should be the word "Recipe" -- I'm not hard coding the word "Recipe" in case this name changes in the file
...
for each_drink in recipe_book:
	...
	for each_ingr in ingr_list:
		...
		if( each_drink[ each_ingr ] is not '' ):
			...
			# [each_ingr] is the looping of every single ingredient
			# each_drink is the whole row from the file, and each_drink[each_ingr] one single ingreidnt amount
			drinks[each_drink[recipe_name]][each_ingr] = each_drink[each_ingr]
		else:
			drinks[each_drink[recipe_name]][each_ingr] = 0
...
def print_recipes_old ( ):
	for each_drink in recipe_book:
		for each_ingr in ingr_list:
			...
			if( each_drink[ each_ingr ] is not '' ):
				print( each_ingr + ': ' + each_drink[ each_ingr ] )
def print_recipes ( ):
	for each_drink in drink_names:
		print each_drink
		for each_ingr in drinks[each_drink]:
			# Skip the ingredients that are not used in this recipe
			# Comment this out of you want empty entries to be printed
			if( drinks[each_drink][ each_ingr ] is not '' ):
				print( each_ingr + ': ' , drinks[each_drink][ each_ingr ] )
# print_recipes ()
...
# Thus, drinks["Calibration"][each_ingr] should be near to 2oz, the amount that pump delivered in 60 seconds
...
calibration_factor={}
for each_ingr in ingr_list:
	if float(drinks["Calibration"][each_ingr]) == not_calibrated:
		yesno = raw_input("Do you want to calibrate pump for " + each_ingr + "?")
		if yesno == "yes":
			calibrate_pump(ingr_pumps[each_ingr])
			new_factor = raw_input( "How much liquid was delivered?")
			calibration_factor[each_ingr]= 1 / float(new_factor) * calibration_seconds
		else:
			print "Well...ok, but that means I'll enter a standard 2oz for this pump and it will be inaccurate!"
			calibration_factor[each_ingr]= 1 / 2 * calibration_seconds
	else:
		calibration_factor[each_ingr]= 1 / float(drinks["Calibration"][each_ingr]) * calibration_seconds

# Check if there's a calibration "drink" -- this "drink" is a multiplier for each pump to equal 1oz
# If there's no calibration "drink" then give a warning, and use a default calibration -- not ideal! Each pump is different!

if drinks.get("Calibration")==None:
	print "NO CALIBRATION!!!!"
	for each_ingr in ingr_list:
		# We assume the pump delivered 2oz in 60 seconds
		drinks["Calibration"][each_ingr] = peristaltic_2oz 
else:
...
while (True):
	...
	for each_drink in drink_names:
		print each_drink
	myinput = raw_input("Enter Drink Name:  ")
	if myinput == "Prime":
		myinput = raw_input("Which pump to prime?  ")
		while myinput not in ingr_list and myinput != "stop":
			print "I don't have a pump for " + myinput
			print "Type stop to not prime a pump."
			myinput == raw_input("Which pump to prime?")
		primeMe(ingr_pumps[myinput])
	elif myinput not in drink_names:
		print "THAT'S NOT A DRINK, YOU SILLY!"
	else:
		for each_ingr in drinks[myinput]:
			if drinks[myinput][each_ingr] > 0:
				print each_ingr + ": " + drinks[myinput][each_ingr]
				print "Normalized: ", float(drinks[myinput][each_ingr]) * calibration_factor[each_ingr], " seconds."
				mixMe( ingr_pumps[each_ingr], float(drinks[myinput][each_ingr]) * calibration_factor[each_ingr])
